...
for (i in 1:nrow(annotations)) {
  ...
  model_data <- data.frame(
    Intensity = trimmed_intensity,
    Radial_distance = 1:length(trimmed_intensity)
    )
  
  fit <- drm(Intensity ~ Radial_distance, data = model_data, fct = LL.3())
  
  # Predict concentrations of samples
  predicted_fit <- predict(fit, newdata = as.data.frame(model_data$Radial_distance))
  
  # Calculate intensity cutoff as per Harvey et. al. method - where intensity is at its maximum.
  Harvey_intensity_cutoff <- which.min(intensity_vector)
  ...
  data.frame(
    ...
    geom_vline(xintercept = Harvey_intensity_cutoff, col = "blue") +
    scale_y_reverse() +
    labs(
  ...
  # calculate area under the curve according to the 4PL model
...
save(area_results, file = paste0(output_dir, "area_results.rda"))
```
